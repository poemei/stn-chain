...
#include "stn_peer.h"
typedef struct stn_windows_peer { uintptr_t socket;uint64_t deadline;int opened; } stn_windows_peer;
/* Explicit IPv4 endpoint, no DNS/discovery. 1..60000 ms total session deadline.
 * Objects must not be copied or concurrently used. Close every successful open.
