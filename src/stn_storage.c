...
stn_storage_status stn_storage_decode(const stn_chain_context *c,const uint8_t *p,size_t n,stn_storage_view *out)
{
    ...
    /*
     * Every stored record requires a length word plus at least a complete
     * canonical block header. The block validator determines whether any
     * transaction body bytes are required by that header.
     */
    if(v.count>(n-STN_STORAGE_OVERHEAD)/(4u+STN_BLOCK_HEADER_SIZE)) { return STN_STORAGE_FORMAT; }
    ...
    for(i=0;i<v.count;++i) {
        ...
        v.blocks[i].bytes=p+offset;
        v.blocks[i].length=length;
        offset+=length;
    }

    if(offset!=n-32) { s=STN_STORAGE_FORMAT;goto fail; }

    s=digest(c,p,offset,hash);
    if(s!=STN_STORAGE_OK) { goto fail; }

    if(memcmp(hash,p+offset,32)!=0) { s=STN_STORAGE_FORMAT;goto fail; }

    s=validate(c,v.blocks,v.count,&v.state);
    if(s!=STN_STORAGE_OK) { goto fail; }
    ...
    uint8_t *p,size_t capacity,size_t *written)
{
    ...
    memcpy(p,"STNS",4);
    stn_wire_write(p+4,2,1);
    stn_wire_write(p+6,2,0);
    stn_wire_write(p+8,4,n);

    for(i=0;i<n;++i) {
        stn_wire_write(p+offset,4,b[i].length);
        offset+=4;
        memcpy(p+offset,b[i].bytes,b[i].length);
        offset+=b[i].length;
    }
    ...
    uint8_t *scratch,size_t capacity,stn_storage_view *out)
{
    ...
    s=read_locked(c,p,scratch,capacity,out);
    p->release(p->user);
    return s;
}
    ...
    const stn_block_span *b,size_t n,uint8_t *scratch,size_t capacity,stn_chain_state *active)
{
    ...
    if(s!=STN_STORAGE_NOT_FOUND) {
        stn_chain_state_release(&state);
        p->release(p->user);
        return s==STN_STORAGE_OK || s==STN_STORAGE_CAPACITY ? STN_STORAGE_EXISTS : s;
    }

    s=stn_storage_encode(c,b,n,scratch,capacity,&length);

    if(s==STN_STORAGE_OK) {
        s=io_status(p->replace(p->user,scratch,length));
    }

    if(s==STN_STORAGE_OK) {
        stn_chain_state_move(active,&state);
    }
    ...
    const stn_block_span *b,size_t n,const stn_reorg_plan *plan,stn_storage_workspace *w,stn_chain_state *active)
{
    ...
    if(s==STN_STORAGE_OK) {
        s=io_status(p->replace(p->user,w->next_bytes,length));
    }

    if(s==STN_STORAGE_OK) {
        stn_chain_state_move(active,&fresh.candidate);
    }

done:
    stn_reorg_plan_release(&fresh);
    ...
    const uint8_t *block,size_t block_length,stn_storage_workspace *w,stn_chain_state *active)
{
    ...
    for(i=0;i<current.count;++i){
        if(offset>SIZE_MAX-4 || current.blocks[i].length>SIZE_MAX-offset-4){
            s=STN_STORAGE_CAPACITY;
            goto done;
        }
    ...
    if(offset>SIZE_MAX-36 || block_length>SIZE_MAX-offset-36){
        s=STN_STORAGE_CAPACITY;
        goto done;
    }
    ...
    if(s==STN_STORAGE_OK){
        memcpy(w->next_bytes+encoded-32,hash,32);
    }

    if(s==STN_STORAGE_OK){
        s=io_status(p->replace(p->user,w->next_bytes,encoded));
    }
    ...
    uint8_t *scratch,size_t capacity,stn_storage_view *out,int *recovery)
{
    ...
    if(c->pow_policy==NULL || stn_chain_initialize(c,&v.state)!=STN_DATA_OK) {
        return STN_STORAGE_VALIDATION;
    }
    ...
    if(s!=STN_STORAGE_FORMAT && s!=STN_STORAGE_VALIDATION) {
        return s;
    }

    if(stn_chain_initialize(c,&v.state)!=STN_DATA_OK){
        return STN_STORAGE_VALIDATION;
    }

    /* Wrong/unknown framing is not guessed. Do not interpret unsupported versions. */
    if(n>=12 &&
       memcmp(scratch,"STNS",4)==0 &&
       stn_wire_read(scratch+4,2)==1 &&
       stn_wire_read(scratch+6,2)==0) {

        count=(size_t)stn_wire_read(scratch+8,4);

        if(count!=0 &&
           count<=SIZE_MAX/sizeof(stn_block_span) &&
           n>=12) {

            /*
             * A truncated tail must not discard a valid prefix. A legal
             * block may consist solely of its canonical header, so size
             * the recovery table using that minimum representation.
             */
            if(count>(n-12)/(4u+STN_BLOCK_HEADER_SIZE)){
                count=(n-12)/(4u+STN_BLOCK_HEADER_SIZE);
            }

            v.blocks=count ?
                (stn_block_span*)calloc(count,sizeof(*v.blocks)) :
                NULL;

            if(count!=0 && v.blocks==NULL){
                stn_storage_view_release(&v);
                return STN_STORAGE_CAPACITY;
            }
        } else {
            count=0;
        }
    ...
    for(i=0;i<count;++i) {
        ...
        if(offset>n || n-offset<4) {
            break;
        }

        length=(size_t)stn_wire_read(scratch+offset,4);
        offset+=4;
        ...
        r=stn_chain_validate_candidate(
            c,
            &v.state,
            scratch+offset,
            length,
        ...
        v.blocks[v.count].bytes=scratch+offset;
        v.blocks[v.count].length=length;
        ++v.count;
        offset+=length;
    }
    ...
    uint8_t *scratch,size_t capacity,stn_storage_view *out,int *recovery)
{
    ...
    if(c==NULL || !provider_valid(p) || scratch==NULL || out==NULL || recovery==NULL) {
    ...
    s=recovery_locked(c,p,scratch,capacity,out,recovery);
    p->release(p->user);
    return s;
}
    ...
    const stn_block_span *b,size_t n,stn_storage_workspace *w,stn_chain_state *active)
{
    ...
    if(c==NULL || !provider_valid(p) || w==NULL || active==NULL ||
       w->current_bytes==NULL || w->next_bytes==NULL) {
        return STN_STORAGE_ARGUMENT;
    }

    s=validate(c,b,n,&candidate);
    ...
    if(s!=STN_STORAGE_OK) {
        stn_chain_state_release(&candidate);
        return s;
    }
    ...
    if(local.count!=0) {
        ...
        if(r.result!=STN_FORK_CANDIDATE &&
           !(recovery &&
            ...
            s=r.result==STN_FORK_CURRENT || r.result==STN_FORK_TIE ?
                STN_STORAGE_NOT_PREFERRED :
                r.result==STN_FORK_UNRESOLVED ?
                    STN_STORAGE_UNRESOLVED :
                    STN_STORAGE_VALIDATION;

            goto finished;
        }
    }

    s=stn_storage_encode(
        c,
        b,
        n,
        w->next_bytes,
    ...
    if(s==STN_STORAGE_OK) {
        stn_chain_state_move(active,&candidate);
    }

finished:
    stn_chain_state_release(&candidate);
    stn_reorg_plan_release(&plan);
    stn_storage_view_release(&local);
    p->release(p->user);
    return s;
}