...
static void reset(stn_chain_context *c,size_t count,stn_chain_state *active)
{
    ...
    CHECK(stn_storage_decode(c,mem.bytes,mem.n,&v)==STN_STORAGE_OK);stn_chain_state_move(active,&v.state);stn_storage_view_release(&v);
}
...
static void synchronization(stn_chain_context *c)
{
    ...
    mock m={0};stn_peer_transport t={&m,mock_send,mock_receive};stn_chain_state active={0},before;
    stn_peer_report r={0};size_t old_n;int mode;unsigned split;
    m.c=c;m.count=3;branch(b,bs,99);reset(c,2,&active);
    stn_peer_report_release(&r);r=stn_peer_sync(c,&p,&t,&w,&active);
    CHECK(r.status==STN_PEER_OK && r.reused_blocks==2 && r.received_blocks==1 && active.height==2);
    {
        ...
        mem.fail=1;stn_peer_report_release(&r);r=stn_peer_sync(c,&p,&t,&w,&active);
        CHECK(r.status==STN_PEER_STORAGE && memcmp(&pending,&pending_before,sizeof(pending))==0);mem.fail=0;
        m.mode=9;memset(&m.session,0,sizeof(m.session));stn_peer_report_release(&r);r=stn_peer_sync(c,&p,&t,&w,&active);
        CHECK(r.status!=STN_PEER_OK && memcmp(&pending,&pending_before,sizeof(pending))==0);
        m.mode=0;memset(&m.session,0,sizeof(m.session));stn_peer_report_release(&r);r=stn_peer_sync(c,&p,&t,&w,&active);
        CHECK(r.status==STN_PEER_OK && pending.count==1 && pending.bytes==192 && memcmp(pending.entries[0].id,kept,32)==0);
        pending_before=pending;memset(&m.session,0,sizeof(m.session));stn_peer_report_release(&r);r=stn_peer_sync(c,&p,&t,&w,&active);
        CHECK(r.status==STN_PEER_RETAINED && memcmp(&pending,&pending_before,sizeof(pending))==0);
        m.count=4;memset(&m.session,0,sizeof(m.session));stn_peer_report_release(&r);r=stn_peer_sync(c,&p,&t,&w,&active);
        CHECK(r.status==STN_PEER_OK && memcmp(&pending,&pending_before,sizeof(pending))==0);
    ...
    before=active;stn_peer_report_release(&r);r=stn_peer_sync(c,&p,&t,&w,&active);
    ...
    for(split=1;split<=4;++split){
        ...
        m.count=5;memset(&m.session,0,sizeof(m.session));stn_peer_report_release(&r);r=stn_peer_sync(c,&p,&t,&w,&active);CHECK(r.status==STN_PEER_RETAINED);
    }
    reset(c,5,&active);branch(b,bs,1);m.count=5;memset(&m.session,0,sizeof(m.session));
    before=active;stn_peer_report_release(&r);r=stn_peer_sync(c,&p,&t,&w,&active);CHECK(r.status==STN_PEER_RETAINED && memcmp(active.tip_id,before.tip_id,32)==0);
    for(mode=2;mode<=15;++mode){
        ...
        stn_peer_report_release(&r);r=stn_peer_sync(c,&p,&t,&w,&active);hash_status=STN_DATA_OK;
    ...
    stn_peer_report_release(&r);r=stn_peer_sync(c,&p,&t,&w,&active);CHECK(r.status==STN_PEER_STORAGE && memcmp(&active,&before,sizeof(active))==0 && mem.n==old_n && memcmp(mem.bytes,saved,old_n)==0);mem.fail=0;
    ...
    stn_chain_state_release(&active);CHECK(stn_chain_initialize(c,&active)==STN_DATA_OK);before=active;m.count=6;branch(b,bs,99);m.mode=9;memset(&m.session,0,sizeof(m.session));
    stn_peer_report_release(&r);r=stn_peer_sync(c,&p,&t,&w,&active);CHECK(r.recovery && r.reused_blocks==3 && r.status!=STN_PEER_OK);
    ...
    mem.bytes[12+4+363]^=1;stn_chain_state_release(&active);CHECK(stn_chain_initialize(c,&active)==STN_DATA_OK);
    ...
    mem.fail=0;memset(&m.session,0,sizeof(m.session));stn_peer_report_release(&r);r=stn_peer_sync(c,&p,&t,&w,&active);CHECK(r.status==STN_PEER_OK && r.reused_blocks==0);
    /* Bad root framing requires full peer evidence, not guessing disk offsets. */
    mem.bytes[0]=0;stn_chain_state_release(&active);CHECK(stn_chain_initialize(c,&active)==STN_DATA_OK);m.count=3;memset(&m.session,0,sizeof(m.session));
    stn_peer_report_release(&r);r=stn_peer_sync(c,&p,&t,&w,&active);CHECK(r.status==STN_PEER_OK && r.recovery && r.reused_blocks==0 && r.received_blocks==3);
    /* Checksum-only corruption preserves all independently validated blocks. */
    mem.bytes[mem.n-1]^=1;stn_chain_state_release(&active);CHECK(stn_chain_initialize(c,&active)==STN_DATA_OK);memset(&m.session,0,sizeof(m.session));
    stn_peer_report_release(&r);r=stn_peer_sync(c,&p,&t,&w,&active);CHECK(r.status==STN_PEER_OK && r.recovery && r.reused_blocks==3 && r.received_blocks==0);
    /* Many weaker peers cannot overturn stronger independently validated evidence. */
    m.count=6;memset(&m.session,0,sizeof(m.session));stn_peer_report_release(&r);r=stn_peer_sync(c,&p,&t,&w,&active);CHECK(r.status==STN_PEER_OK);
    before=active;m.count=2;m.mode=1;
    for(mode=0;mode<3;++mode){memset(&m.session,0,sizeof(m.session));stn_peer_report_release(&r);r=stn_peer_sync(c,&p,&t,&w,&active);CHECK(r.status==STN_PEER_RETAINED && memcmp(&active,&before,sizeof(active))==0);}
    /* Missing store root bootstrap and maximum bounded count. */
    mem.exists=0;m.mode=0;m.count=64;memset(&m.session,0,sizeof(m.session));stn_chain_state_release(&active);CHECK(stn_chain_initialize(c,&active)==STN_DATA_OK);
    stn_peer_report_release(&r);r=stn_peer_sync(c,&p,&t,&w,&active);CHECK(r.status==STN_PEER_OK && active.height==63 && r.received_blocks==64 && m.session.requests==67);    /* Header pagination, deep catch-up, reconnect, reorg and truncated-prefix
    ...
    stn_peer_report_release(&r);r=stn_peer_sync(c,&p,&t,&w,&active);CHECK(r.status==STN_PEER_RETAINED && memcmp(&active,&before,sizeof(active))==0);
    reset(c,100,&active);branch(b,bs,80);m.count=130;m.mode=4;memset(&m.session,0,sizeof(m.session));before=active;
    stn_peer_report_release(&r);r=stn_peer_sync(c,&p,&t,&w,&active);CHECK(r.status==STN_PEER_DISCONNECTED && memcmp(&active,&before,sizeof(active))==0);
    m.mode=0;memset(&m.session,0,sizeof(m.session));stn_peer_report_release(&r);r=stn_peer_sync(c,&p,&t,&w,&active);
    ...
    stn_peer_report_release(&r);stn_chain_state_release(&active);
}
...
static void localhost(stn_chain_context *c)
{
    ...
    stn_storage_provider p={&mem,lock_mem,unlock_mem,read_mem,write_mem};
    stn_peer_workspace w={{scratch,CAP,next_bytes,CAP},candidate_bytes,CAP,frame,sizeof(frame)};stn_chain_state active={0};stn_peer_report r={0};
...
static void convergence(void)
{
    ...
    stn_chain_state states[2]={{0}},before[2],prefix;stn_peer_report report={0};stn_reorg_plan plan={0};stn_fork_report fork;
    ...
    for(high=0;high<2;++high){
        ...
        stn_peer_report_release(&report);report=exchange_branch(&c,as,1,NULL,&providers[1],&workspace,&states[1]);CHECK(report.status==STN_PEER_RETAINED);
        prefix=states[0];
        if(!high){
        ...
        stn_reorg_plan_release(&plan);fork=stn_fork_evaluate_history(&c,bs,61,as,61,&plan);CHECK(fork.result==STN_FORK_CANDIDATE && plan.ancestor_index==0 && plan.detached_count==60 && plan.attached_count==60);
        ...
        stn_peer_report_release(&report);report=exchange_branch(&c,bs,61,wrong,&providers[0],&workspace,&states[0]);CHECK(report.status==STN_PEER_VALIDATION && memcmp(&states[0],&before[0],sizeof(states[0]))==0);
        ...
        stn_peer_report_release(&report);report=exchange_branch(&c,bs,61,NULL,&providers[0],&workspace,&states[0]);CHECK(report.status==STN_PEER_RETAINED && memcmp(states[0].cumulative_work.bytes,before[0].cumulative_work.bytes,40)==0);
        stn_peer_report_release(&report);report=exchange_branch(&c,as,61,NULL,&providers[1],&workspace,&states[1]);CHECK(report.status==STN_PEER_OK && report.reused_blocks==1 && report.received_blocks==60);
        ...
        for(j=0;j<2;++j){
            stn_chain_state_release(&states[j]); /* reopen canonical storage */
            CHECK(stn_windows_storage_init(&disks[j],paths[j],&providers[j])==STN_STORAGE_OK);
            CHECK(stn_storage_load(&c,&providers[j],snapshot,CAP,&view)==STN_STORAGE_OK);
            stn_chain_state_move(&states[j],&view.state);CHECK(view.count==61 && memcmp(states[j].current_target,before[0].current_target,32)==0 && memcmp(states[j].tip_id,before[0].tip_id,32)==0 && memcmp(states[j].cumulative_work.bytes,before[0].cumulative_work.bytes,40)==0);
...
static void outbound_mock(stn_chain_context *c)
{
    ...
    stn_chain_state active={0},before;uint64_t tick=0;unsigned start=checks,i;
    ...
    stn_chain_state_release(&active);
...
static void outbound_actual(stn_chain_context *c)
{
    stn_storage_provider storage={&mem,lock_mem,unlock_mem,read_mem,write_mem};
    stn_peer_workspace workspace={{scratch,CAP,next_bytes,CAP},candidate_bytes,CAP,frame,sizeof(frame)};
    stn_chain_state active={0},before;unsigned start=checks,scenario;size_t i;
    for(scenario=0;scenario<2;++scenario){
        ...
        {stn_peer_report retained=stn_peer_sync(c,&storage,&inbound_transport,&workspace,&active);CHECK(retained.status==STN_PEER_RETAINED);stn_peer_report_release(&retained);}
...
static void discovery(stn_chain_context *c)
{
    ...
    /* Discovery cannot bypass Block 2 or ordinary evidence validation. */
    {
        ...
        stn_storage_provider storage={&mem,lock_mem,unlock_mem,read_mem,write_mem};
        stn_peer_workspace workspace={{scratch,CAP,next_bytes,CAP},candidate_bytes,CAP,frame,sizeof(frame)};
        stn_chain_state active={0},accepted;size_t requests;
        ...
        stn_peer_outbound_close(&manager);stn_chain_state_release(&active);
    }
...
static void discovery_actual(stn_chain_context *c)
{
    ...
    stn_windows_peer socket={0};stn_peer_connector connector={&socket,stn_windows_peer_open_candidate,stn_windows_peer_close_candidate};stn_peer_outbound manager;
    stn_storage_provider storage={&mem,lock_mem,unlock_mem,read_mem,write_mem};
    stn_peer_workspace workspace={{scratch,CAP,next_bytes,CAP},candidate_bytes,CAP,frame,sizeof(frame)};stn_chain_state active={0},before;
...
static void candidates(const stn_chain_context *context)
{
    ...
    stn_chain_state_release(&empty);stn_chain_state_release(&accepted_before);stn_chain_state_release(&accepted_after);
