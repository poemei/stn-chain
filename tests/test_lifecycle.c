...
#include "test_publication.c"
#include "test_record_query.c"
...
{
    record_queries();
    printf("Accepted-record targeted: %u checks, %u failures.\n",checks,failures);
    return failures!=0;
}
int test_publication_ownership(void)
{
    ...
    printf("Publication ownership: %u checks, %u failures.\n",checks,failures);
    return failures!=0;
}