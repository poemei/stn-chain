...
int test_fork(void)
{
    ...
    stn_reorg_plan p={0},before;stn_fork_report r;unsigned split,i;
    ...
    stn_reorg_plan_release(&p);r=stn_fork_evaluate(&c,as,2,bs,3,&p);
    ...
    stn_reorg_plan_release(&p);r=stn_fork_evaluate(&c,as,4,bs,2,&p);
    ...
    stn_reorg_plan_release(&p);r=stn_fork_evaluate(&c,as,4,as,4,&p);
    CHECK(r.result==STN_FORK_TIE && !p.actionable && p.detached_count==0 && p.attached_count==0);
    for(split=1;split<5;++split) {
        branch(b,bs,split);
        stn_reorg_plan_release(&p);r=stn_fork_evaluate(&c,as,5,bs,6,&p);
        ...
        stn_reorg_plan_release(&p);r=stn_fork_evaluate(&c,as,5,bs,5,&p);
    ...
    stn_reorg_plan_release(&p);r=stn_fork_evaluate(&c,as,1,as,2,&p);
