...
static void activated_difficulty(void)
{
    ...
    stn_chain_state state={0},before={0},unchanged;stn_chain_report report;stn_storage_view view;
    stn_storage_provider provider={&store,acquire,release,read_store,replace};
    stn_mining_service s={0};stn_reorg_plan plan={0};stn_fork_report fork;
    ...
    for(k=0;k<sizeof(spans)/sizeof(spans[0]);++k) {
        ...
        CHECK(stn_chain_state_share(&state,&before)==STN_DATA_OK);
        ...
        stn_chain_state_release(&state);CHECK(stn_chain_state_share(&before,&state)==STN_DATA_OK);state.target_history_count=59;memset(out,0xa5,32);
        ...
        stn_chain_state_move(&state,&view.state);stn_storage_view_release(&view);
        stn_chain_state_release(&s.active);
        ...
        if(k==1) {
            ...
            stn_reorg_plan_release(&plan);fork=stn_fork_evaluate_history(&c,other,61,view.blocks,view.count,&plan);
            ...
            stn_chain_state_release(&alt);stn_reorg_plan_release(&plan);
        }
        stn_chain_state_release(&state);stn_chain_state_release(&before);stn_chain_state_release(&s.active);
    }
