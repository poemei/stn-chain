...
static void pow_chain(void)
{
    ...
    stn_chain_context c={0};stn_pow_policy policy;stn_chain_state empty={0},s={0},next={0},before,full={0};
    ...
    stn_chain_state_release(&empty);CHECK(stn_chain_initialize(&c,&empty)==STN_DATA_OK);
    stn_chain_state_release(&s);r=stn_chain_validate_candidate(&c,&empty,genesis,364,&s);
    ...
    memcpy(child,genesis,364);memcpy(child+40,s.tip_id,32);child[79]=1;stn_chain_state_release(&next);before=s;
    r=stn_chain_validate_candidate(&c,&s,child,364,&next);
    ...
    CHECK(stn_chain_initialize(&c,&next)!=STN_DATA_OK && same(&next,&before));
    stn_chain_state_release(&empty);stn_chain_state_release(&s);stn_chain_state_release(&next);stn_chain_state_release(&full);
}
...
static void work_domain(void)
{
    ...
    stn_chain_state state={0},prior,recovered={0};stn_work w,sum,expected={{0}},saved,one={{0}};
    ...
    stn_chain_state_release(&state);CHECK(stn_chain_initialize(&c,&state)==STN_DATA_OK);
    for(i=0;i<61;++i){
    ...
    stn_chain_state_move(&recovered,&view.state);stn_storage_view_release(&view);
    ...
    stn_chain_state_release(&state);stn_chain_state_release(&recovered);
