static const uint8_t reject_action_signature[64]={0x98,0xa2,0x83,0x64,0xa6,0xf6,0x39,0x6b,0x6d,0x0e,0x36,0x97,0xe6,0x00,0xd5,0x98,0xf7,0xbd,0xd4,0x22,0xd3,0x81,0x21,0x30,0x5a,0x55,0x08,0xc5,0xbe,0x8b,0xa8,0x28,0x23,0x82,0x4b,0x1b,0xd4,0x1b,0xae,0x9c,0x21,0xb3,0x64,0x00,0x34,0xba,0x82,0x57,0x85,0x18,0xe5,0x8a,0xaf,0x26,0x7e,0x6d,0xdf,0x3f,0x6a,0xfe,0xa1,0xf8,0xbd,0x0d};
...
static void contract_history_majority(void)
{
    ...
    body=malloc(STN_BLOCK_MAX_BODY);
    CHECK(body!=NULL);
    if(body==NULL)return;
    blocks=malloc(8u*STN_BLOCK_MAX_SIZE);
    CHECK(blocks!=NULL);
    if(blocks==NULL){free(body);return;}

#define BUILD_HISTORY_BLOCK(INDEX,HEIGHT,TXCOUNT) do { \
    CHECK(stn_block_body_encode(&span,(TXCOUNT),body,STN_BLOCK_MAX_BODY,&body_length)==STN_DATA_OK); \
...
#define ACCEPT_HISTORY_BLOCK(INDEX) do { \
    ...
    stn_chain_state_release(&rebuilt);stn_chain_state_release(&state);free(blocks);free(body);
...
static void contract_chain_create(void)
{
    ...
    uint8_t tx_bytes[STN_TX_MAX_SIZE];
    uint8_t *body=NULL;
    uint8_t *genesis_bytes=NULL;
    uint8_t *create_history_bytes=NULL;
    uint8_t *child_bytes=NULL;
    ...
    body=malloc(STN_BLOCK_MAX_BODY);
    genesis_bytes=malloc(STN_BLOCK_MAX_SIZE);
    create_history_bytes=malloc(STN_BLOCK_MAX_SIZE);
    child_bytes=malloc(STN_BLOCK_MAX_SIZE);
    CHECK(body!=NULL && genesis_bytes!=NULL && create_history_bytes!=NULL && child_bytes!=NULL);
    if(body==NULL || genesis_bytes==NULL || create_history_bytes==NULL || child_bytes==NULL){
        free(child_bytes);free(create_history_bytes);free(genesis_bytes);free(body);
        return;
    }
    ...
    span.bytes=tx_bytes;span.length=(uint32_t)tx_length;
    CHECK(stn_block_body_encode(&span,1u,body,STN_BLOCK_MAX_BODY,&body_length)==STN_DATA_OK);
    ...
    CHECK(stn_block_encode(&block,genesis_bytes,STN_BLOCK_MAX_SIZE,&genesis_length)==STN_DATA_OK);
    ...
    CHECK(stn_block_body_commitment(body,body_length,1u,&provider,
        block.header.transaction_commitment)==STN_DATA_OK);
    CHECK(stn_block_encode(&block,child_bytes,STN_BLOCK_MAX_SIZE,&child_length)==STN_DATA_OK);

    report=stn_chain_validate_candidate(&context,&genesis_state,child_bytes,child_length,&accepted);
    CHECK(report.acceptance==STN_ACCEPTANCE_UNDER_CONTEXT);
    CHECK(child_length<=STN_BLOCK_MAX_SIZE);
...

    {
        ...
        {
            stn_transaction_span ordered[2];
            stn_chain_state wrong_order={0},right_order={0};

            ordered[0].bytes=tx_bytes;ordered[0].length=(uint32_t)tx_length;
            ordered[1].bytes=amend_grant_tx;ordered[1].length=(uint32_t)amend_grant_tx_length;
            CHECK(stn_block_body_encode(ordered,2u,body,STN_BLOCK_MAX_BODY,&body_length)==STN_DATA_OK);
            memset(&block,0,sizeof(block));block.header.version=1u;block.header.network_id[0]=1u;
            memcpy(block.header.previous_hash,accepted.tip_id,32);block.header.height=2u;block.header.timestamp=2u;
            block.header.transaction_count=2u;block.header.body_length=(uint32_t)body_length;block.body=body;
            CHECK(stn_block_body_commitment(body,body_length,2u,&provider,
                block.header.transaction_commitment)==STN_DATA_OK);
            CHECK(stn_block_encode(&block,child_bytes,STN_BLOCK_MAX_SIZE,&child_length)==STN_DATA_OK);
            report=stn_chain_validate_candidate(&context,&accepted,child_bytes,child_length,&wrong_order);
            CHECK(report.acceptance!=STN_ACCEPTANCE_UNDER_CONTEXT);
            CHECK(stn_contract_snapshot_const_state(accepted.contracts)->entries[0].current.state==
                STN_CONTRACT_STATE_ISSUED);
            CHECK(stn_contract_snapshot_const_state(accepted.contracts)->entries[0].current.sequence==1u);
            stn_chain_state_release(&wrong_order);

            ordered[0].bytes=amend_grant_tx;ordered[0].length=(uint32_t)amend_grant_tx_length;
            ordered[1].bytes=tx_bytes;ordered[1].length=(uint32_t)tx_length;
            CHECK(stn_block_body_encode(ordered,2u,body,STN_BLOCK_MAX_BODY,&body_length)==STN_DATA_OK);
            memset(&block,0,sizeof(block));block.header.version=1u;block.header.network_id[0]=1u;
            memcpy(block.header.previous_hash,accepted.tip_id,32);block.header.height=2u;block.header.timestamp=2u;
            block.header.transaction_count=2u;block.header.body_length=(uint32_t)body_length;block.body=body;
            CHECK(stn_block_body_commitment(body,body_length,2u,&provider,
                block.header.transaction_commitment)==STN_DATA_OK);
            CHECK(stn_block_encode(&block,child_bytes,STN_BLOCK_MAX_SIZE,&child_length)==STN_DATA_OK);
            report=stn_chain_validate_candidate(&context,&accepted,child_bytes,child_length,&right_order);
            CHECK(report.acceptance==STN_ACCEPTANCE_UNDER_CONTEXT);
            CHECK(right_order.lifecycle!=NULL && right_order.lifecycle->grant_count==2u);
            CHECK(stn_contract_snapshot_const_state(right_order.contracts)->entries[0].current.state==
                STN_CONTRACT_STATE_REVIEW);
            CHECK(stn_contract_snapshot_const_state(right_order.contracts)->entries[0].current.sequence==2u);
            CHECK(stn_contract_snapshot_const_state(accepted.contracts)->entries[0].current.state==
                STN_CONTRACT_STATE_ISSUED);
            CHECK(stn_contract_snapshot_const_state(accepted.contracts)->entries[0].current.sequence==1u);
            stn_chain_state_release(&right_order);
        }
        ...
        {
            stn_contract review_revoke=state->entries[0].current;
            stn_chain_state revoke_grant_state={0},revoked_state={0};
            uint8_t review_revoke_bytes[STN_CONTRACT_MAX_SIZE];
            uint8_t revoke_evidence[STN_AUTHORITY_EVIDENCE_SIZE];
            uint8_t revoke_grant[STN_AUTHORITY_GRANT_SIZE];
            size_t review_revoke_length=0,revoke_evidence_length=0,revoke_grant_length=0;

            review_revoke.participants=participants;review_revoke.participant_bytes=NULL;
            CHECK(stn_contract_encode(&review_revoke,review_revoke_bytes,sizeof(review_revoke_bytes),
                &review_revoke_length)==STN_CONTRACT_OK);
            CHECK(stn_contract_authority_action(STN_CONTRACT_ACTION_REVOKE,authority_action)==STN_CONTRACT_OK);
            CHECK(stn_contract_authority_context(draft_bytes,draft_length,authority_context)==STN_CONTRACT_OK);
            CHECK(stn_authority_evidence_encode(chain_root,authority_action,authority_context,
            ...
            CHECK(stn_block_body_commitment(body,body_length,1u,&provider,
                block.header.transaction_commitment)==STN_DATA_OK);
            CHECK(stn_block_encode(&block,child_bytes,STN_BLOCK_MAX_SIZE,&child_length)==STN_DATA_OK);
            report=stn_chain_validate_candidate(&context,&review_state,child_bytes,child_length,&revoke_grant_state);
            CHECK(report.acceptance==STN_ACCEPTANCE_UNDER_CONTEXT);

            action.action=STN_CONTRACT_ACTION_REVOKE;action.sequence=3u;
            ...
            CHECK(stn_block_body_encode(&span,1u,body,STN_BLOCK_MAX_BODY,&body_length)==STN_DATA_OK);
            memset(&block,0,sizeof(block));block.header.version=1u;block.header.network_id[0]=1u;
            memcpy(block.header.previous_hash,revoke_grant_state.tip_id,32);block.header.height=5u;block.header.timestamp=5u;
            block.header.transaction_count=1u;block.header.body_length=(uint32_t)body_length;block.body=body;
            CHECK(stn_block_body_commitment(body,body_length,1u,&provider,
                block.header.transaction_commitment)==STN_DATA_OK);
            CHECK(stn_block_encode(&block,child_bytes,STN_BLOCK_MAX_SIZE,&child_length)==STN_DATA_OK);
            report=stn_chain_validate_candidate(&context,&revoke_grant_state,child_bytes,child_length,&revoked_state);
            CHECK(report.acceptance==STN_ACCEPTANCE_UNDER_CONTEXT);
            state=stn_contract_snapshot_state(revoked_state.contracts);
            CHECK(state->entries[0].current.state==STN_CONTRACT_STATE_REVOKED);
            CHECK(state->entries[0].current.sequence==3u);
            CHECK(state->entries[0].vote_count==0u);
            CHECK(memcmp(state->entries[0].contract_id,address.identifier,STN_ADDRESS_ID_SIZE)==0);
            CHECK(stn_contract_snapshot_const_state(review_state.contracts)->entries[0].current.state==
                STN_CONTRACT_STATE_REVIEW);
            CHECK(stn_contract_snapshot_const_state(review_state.contracts)->entries[0].current.sequence==2u);
            stn_chain_state_release(&revoke_grant_state);
            stn_chain_state_release(&revoked_state);
            state=stn_contract_snapshot_state(review_state.contracts);
        }

        {
            stn_contract review_reject=state->entries[0].current;
            stn_chain_state reject_grant_state={0},rejected_state={0};
            uint8_t review_reject_bytes[STN_CONTRACT_MAX_SIZE];
            uint8_t reject_evidence[STN_AUTHORITY_EVIDENCE_SIZE];
            uint8_t reject_grant[STN_AUTHORITY_GRANT_SIZE];
            size_t review_reject_length=0,reject_evidence_length=0,reject_grant_length=0;

            review_reject.participants=participants;review_reject.participant_bytes=NULL;
            CHECK(stn_contract_encode(&review_reject,review_reject_bytes,sizeof(review_reject_bytes),
                &review_reject_length)==STN_CONTRACT_OK);
            CHECK(stn_contract_authority_action(STN_CONTRACT_ACTION_REJECT,authority_action)==STN_CONTRACT_OK);
            CHECK(stn_contract_authority_context(draft_bytes,draft_length,authority_context)==STN_CONTRACT_OK);
            CHECK(stn_authority_evidence_encode(chain_root,authority_action,authority_context,
            ...
            CHECK(stn_block_body_commitment(body,body_length,1u,&provider,
                block.header.transaction_commitment)==STN_DATA_OK);
            CHECK(stn_block_encode(&block,child_bytes,STN_BLOCK_MAX_SIZE,&child_length)==STN_DATA_OK);
            report=stn_chain_validate_candidate(&context,&review_state,child_bytes,child_length,&reject_grant_state);
            CHECK(report.acceptance==STN_ACCEPTANCE_UNDER_CONTEXT);

            action.action=STN_CONTRACT_ACTION_REJECT;action.sequence=3u;
            ...
            CHECK(stn_block_body_encode(&span,1u,body,STN_BLOCK_MAX_BODY,&body_length)==STN_DATA_OK);
            memset(&block,0,sizeof(block));block.header.version=1u;block.header.network_id[0]=1u;
            memcpy(block.header.previous_hash,reject_grant_state.tip_id,32);block.header.height=5u;block.header.timestamp=5u;
            block.header.transaction_count=1u;block.header.body_length=(uint32_t)body_length;block.body=body;
            CHECK(stn_block_body_commitment(body,body_length,1u,&provider,
                block.header.transaction_commitment)==STN_DATA_OK);
            CHECK(stn_block_encode(&block,child_bytes,STN_BLOCK_MAX_SIZE,&child_length)==STN_DATA_OK);
            report=stn_chain_validate_candidate(&context,&reject_grant_state,child_bytes,child_length,&rejected_state);
            CHECK(report.acceptance==STN_ACCEPTANCE_UNDER_CONTEXT);
            state=stn_contract_snapshot_state(rejected_state.contracts);
            CHECK(state->entries[0].current.state==STN_CONTRACT_STATE_REJECTED);
            CHECK(state->entries[0].current.sequence==3u);
            CHECK(state->entries[0].vote_count==0u);
            CHECK(memcmp(state->entries[0].contract_id,address.identifier,STN_ADDRESS_ID_SIZE)==0);
            CHECK(stn_contract_snapshot_const_state(review_state.contracts)->entries[0].current.state==
            ...
            CHECK(stn_contract_snapshot_const_state(review_state.contracts)->entries[0].current.sequence==2u);
            {
                ...
                CHECK(stn_block_body_encode(&span,1u,body,STN_BLOCK_MAX_BODY,&body_length)==STN_DATA_OK);
                memset(&block,0,sizeof(block));block.header.version=1u;block.header.network_id[0]=1u;
                memcpy(block.header.previous_hash,rejected_state.tip_id,32);block.header.height=6u;block.header.timestamp=6u;
                block.header.transaction_count=1u;block.header.body_length=(uint32_t)body_length;block.body=body;
                CHECK(stn_block_body_commitment(body,body_length,1u,&provider,
                    block.header.transaction_commitment)==STN_DATA_OK);
                CHECK(stn_block_encode(&block,child_bytes,STN_BLOCK_MAX_SIZE,&child_length)==STN_DATA_OK);
                report=stn_chain_validate_candidate(&context,&rejected_state,child_bytes,child_length,&close_grant_state);
                CHECK(report.acceptance==STN_ACCEPTANCE_UNDER_CONTEXT);

                action.action=STN_CONTRACT_ACTION_CLOSE;action.sequence=4u;
                ...
                CHECK(stn_block_body_encode(&span,1u,body,STN_BLOCK_MAX_BODY,&body_length)==STN_DATA_OK);
                memset(&block,0,sizeof(block));block.header.version=1u;block.header.network_id[0]=1u;
                memcpy(block.header.previous_hash,close_grant_state.tip_id,32);block.header.height=7u;block.header.timestamp=7u;
                block.header.transaction_count=1u;block.header.body_length=(uint32_t)body_length;block.body=body;
                CHECK(stn_block_body_commitment(body,body_length,1u,&provider,
                    block.header.transaction_commitment)==STN_DATA_OK);
                CHECK(stn_block_encode(&block,child_bytes,STN_BLOCK_MAX_SIZE,&child_length)==STN_DATA_OK);
                report=stn_chain_validate_candidate(&context,&close_grant_state,child_bytes,child_length,&closed_state);
                CHECK(report.acceptance==STN_ACCEPTANCE_UNDER_CONTEXT);
                state=stn_contract_snapshot_state(closed_state.contracts);
                CHECK(state->entries[0].current.state==STN_CONTRACT_STATE_CLOSED);
                CHECK(state->entries[0].current.sequence==4u);
                CHECK(state->entries[0].vote_count==0u);
                CHECK(memcmp(state->entries[0].contract_id,address.identifier,STN_ADDRESS_ID_SIZE)==0);
                CHECK(stn_contract_snapshot_const_state(rejected_state.contracts)->entries[0].current.state==
                    STN_CONTRACT_STATE_REJECTED);
                CHECK(stn_contract_snapshot_const_state(rejected_state.contracts)->entries[0].current.sequence==3u);
                stn_chain_state_release(&close_grant_state);
                stn_chain_state_release(&closed_state);
                state=stn_contract_snapshot_state(rejected_state.contracts);
            }

            stn_chain_state_release(&reject_grant_state);
            stn_chain_state_release(&rejected_state);
            state=stn_contract_snapshot_state(review_state.contracts);
        }

        {
...

            {
                ...
                {
                    stn_chain_state invalid_grant_state={0};
                    uint8_t invalid_execute_grant[STN_AUTHORITY_GRANT_SIZE];
                    memcpy(invalid_execute_grant,execute_grant,execute_grant_length);
                    invalid_execute_grant[STN_AUTHORITY_GRANT_SIZE-1u]^=1u;
                    grant_tx.record_bytes=invalid_execute_grant;
                    grant_tx.record_length=(uint32_t)execute_grant_length;
                    CHECK(stn_transaction_encode(&grant_tx,tx_bytes,sizeof(tx_bytes),&tx_length)==STN_DATA_OK);
                    span.bytes=tx_bytes;span.length=(uint32_t)tx_length;
                    CHECK(stn_block_body_encode(&span,1u,body,STN_BLOCK_MAX_BODY,&body_length)==STN_DATA_OK);
                    memset(&block,0,sizeof(block));block.header.version=1u;block.header.network_id[0]=1u;
                    memcpy(block.header.previous_hash,approval_two.tip_id,32);
                    block.header.height=8u;block.header.timestamp=8u;
                    block.header.transaction_count=1u;block.header.body_length=(uint32_t)body_length;block.body=body;
                    CHECK(stn_block_body_commitment(body,body_length,1u,&provider,
                        block.header.transaction_commitment)==STN_DATA_OK);
                    CHECK(stn_block_encode(&block,child_bytes,STN_BLOCK_MAX_SIZE,&child_length)==STN_DATA_OK);
                    report=stn_chain_validate_candidate(&context,&approval_two,
                        child_bytes,child_length,&invalid_grant_state);
                    CHECK(report.acceptance!=STN_ACCEPTANCE_UNDER_CONTEXT);
                    CHECK(stn_contract_snapshot_const_state(approval_two.contracts)->entries[0].current.state==
                        STN_CONTRACT_STATE_ATTESTATION);
                    CHECK(stn_contract_snapshot_const_state(approval_two.contracts)->entries[0].current.sequence==4u);
                    stn_chain_state_release(&invalid_grant_state);
                }
